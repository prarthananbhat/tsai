...
from custom_transform import NewToTensor, NewNormalise
from accuracy import showMeasurePlots
from bg_sub_model_6channel import ResNet18
from trainmodel import trainModel
from testmodel import testModel
import albumentations as A
from transform import AlbumentationTransforms
...
img_transforms = [A.RGBShift(r_shift_limit=50, g_shift_limit=50, b_shift_limit=50, p=0.5),
    A.Normalize(mean=channel_means, std=channel_stdevs)]
bg_transforms = [A.RGBShift(r_shift_limit=50, g_shift_limit=50, b_shift_limit=50, p=0.5),
    A.Normalize(mean=channel_means, std=channel_stdevs)]
target_transforms = []
train_transform = AlbumentationTransforms(img_transforms,bg_transforms,target_transforms)

test_img_transforms = [A.Normalize(mean=channel_means, std=channel_stdevs)]
test_bg_transforms = [A.Normalize(mean=channel_means, std=channel_stdevs)]
test_target_transforms = []
test_transform = AlbumentationTransforms(test_img_transforms,test_bg_transforms,test_target_transforms)
